...
#define TAG "com.pushbots.android"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Pushbots_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Pushbots.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::PushbotsBindings::lookupGeneratedInit(
...
static void Pushbots_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Pushbots_getBinding)->GetFunction());
}

static void Pushbots_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::PushbotsBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry PushbotsBinding = {
	"com.pushbots.android",
	Pushbots_init,
	Pushbots_dispose
};
...
Java_com_pushbots_android_PushbotsBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.pushbots.android", &PushbotsBinding);
	titanium::KrollBindings::addExternalLookup(&(::PushbotsBindings::lookupGeneratedInit));
}
